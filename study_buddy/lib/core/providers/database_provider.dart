...
Future<StudyDatabase> _openDb(String path) async {
  if (Platform.isAndroid || Platform.isIOS) {
    return StudyDatabase.open(factory: sqflite.databaseFactory, path: path);
  }
  sqfliteFfiInit();
  return StudyDatabase.open(factory: databaseFactoryFfi, path: path);
}

final databaseProvider = FutureProvider<StudyDatabase>((ref) async {
  final dir = await getApplicationSupportDirectory();
  final path = p.join(dir.path, 'study_buddy.db');
  final sdb = await _openDb(path);
